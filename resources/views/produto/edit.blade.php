...
@section('content')
    <div class="w-full">
        <div class="bg-purple-700 w-full flex justify-between rounded-md p-4">
            <div class="font-bold text-white">Editar produto</div>
    ...
    <form class="bg-gray-200 mt-4 p-4 grid grid-cols-2 gap-4 shadow-md rounded-md" 
        method="POST"
        action="{{ route('produto.salvar',["product" => $product->id]) }}">
        @csrf
        @method('PUT')
        <div>
        ...
        <div>
            <label class="flex flex-col">
                Descrição
                <textarea name="description" id="description" class="border py-2 rounded-md shadow-sm">{{$product->description}}</textarea>
            </label>
        </div>
        <div>
            <label class="flex flex-col">
